...
(function () {
  ...
  function searchBooks(query) {
    ...
    if (query.length < 2) {
      resultsEl.innerHTML =
        '<div class="text-center py-4 text-muted small" role="status">Type to search books...</div>';
      return;
    }
    ...
    searchTimeout = setTimeout(function () {
      resultsEl.innerHTML =
        '<div class="text-center py-4 text-muted small" role="status" aria-label="Searching"><div class="spinner-border spinner-border-sm me-2"></div>Searching...</div>';
      ...
      fetch("/api/search?q=" + encodeURIComponent(query))
        ...
        .then(function (data) {
          if (
            ...
            resultsEl.innerHTML =
              '<div class="text-center py-4 text-muted small" role="status">No results found for "' +
              esc(query) +
              '"</div>';
            return;
          }
          ...
          resultsEl.innerHTML = items
            ...
            .map(function (item) {
              ...
              return (
                '<div class="sr-item" role="option" onclick="window.location.href=\'/books/' +
                id +
                "'\" onkeydown=\"if(event.key==='Enter')window.location.href='/books/" +
                id +
                '\'" tabindex="0">' +
                ...
                '<div style="flex:1;min-width:0;"><div class="fw-bold small">' +
                esc(title) +
        ...
        .catch(function () {
          resultsEl.innerHTML =
            '<div class="text-center py-4 text-muted small" role="status">Search error. Please try again.</div>';
        });
    }, 300);
  }
