...
(function () {
  ...
  function showSkeleton(containerId) {
    ...
    if (!c) return;
    c.innerHTML =
      '<div class="skeleton-post" role="status" aria-label="Loading">' +
      '<div class="skeleton-avatar"></div>' +
      '<div class="skeleton-content"><div class="skeleton-line w60"></div><div class="skeleton-line"></div><div class="skeleton-line w40"></div></div></div>'.repeat(
        3,
      );
  }
  ...
  function showError(containerId, message) {
    ...
    c.innerHTML =
      ...
      '<div class="empty-desc">' +
      (message || "Couldn't load posts. Please try again.") +
      "</div>" +
  ...
  function renderPost(post) {
    return (
      '<article class="post-card animate-d1" aria-label="Post by @' +
      post.user_id +
      '">' +
      ...
      post.user_id +
      "</small>" +
      '<small style="color:var(--text-dim);margin-left:auto">' +
      ...
      (post.book_id
        ? '<a href="/books/' +
          post.book_id +
          '" class="vibe-tag">📚 ' +
          escapeHtml(post.book_title || "Book") +
          "</a>"
        : "") +
      '<footer class="post-actions">' +
      '<button class="post-action" onclick="booktaleFeed.likePost(\'' +
      post.post_id +
      '\',this)" aria-label="Like" aria-pressed="false">♥ <span>' +
      (post.likes || 0) +
      "</span></button>" +
      '<button class="post-action" onclick="window.location.href=\'/feed#comment-' +
      post.post_id +
      '\'" aria-label="Comment">💬 <span>' +
      (post.comments || 0) +
      "</span></button>" +
      '<button class="post-action" onclick="booktaleFeed.repost(\'' +
      post.post_id +
      '\',this)" aria-label="Repost" aria-pressed="false">↻</button>' +
      "</footer></div></article>"
    );
  }
  ...
  function handleEmptyTab(tab, containerId) {
    ...
    if (tab === "following") {
      icon = "👥";
      title = "Follow some readers";
      desc = "Follow some readers to see their posts here";
      ctaText = "Explore Readers →";
      ctaHref = "/explore";
    } else if (tab === "trending") {
      icon = "🔥";
      title = "No trending posts yet";
      desc = "Be the first to post something great";
      ctaText = "Be the first to post";
      ctaHref = "#";
    } else {
      icon = "🔍";
      title = "Discover readers";
      desc = "Find readers with similar interests";
      ctaText = "Find Readers";
      ctaHref = "/search?entity=users";
    }
  ...
  function repost(postId, btn) {
    fetch("/api/posts/" + postId + "/repost", { method: "POST" })
      ...
      .then(function (data) {
        ...
        if (window.showToast)
          showToast(isReposted ? "Reposted!" : "Repost removed", "success");
      })
      .catch(function (err) {
        if (window.showToast) showToast(err.message || "Error", "error");
      });
