...
(function () {
  ...
  function toggleCompanion() {
    ...
    if (isOpen) {
      ...
      if (drawer.querySelector(".ai-message.bot") === null) {
        addBotMessage(
          "Hi! I'm your AI Reading Companion. Ask me about book recommendations, summaries, or discussion questions!",
        );
      }
    }
  }
  ...
  function sendMessage() {
    ...
    if (messagesDiv) {
      messagesDiv.innerHTML +=
        '<div class="ai-message bot" style="font-size:.8rem;color:var(--text-muted);"><div class="spinner-border spinner-border-sm me-2"></div>Thinking...</div>';
    ...
    fetch("/api/ai/chat", {
      ...
      .then(function (data) {
        ...
        if (thinking && thinking.textContent.includes("Thinking"))
          thinking.remove();
        ...
        addBotMessage(
          data.response ||
            data.message ||
            "I'm not sure how to answer that. Try asking about book recommendations!",
        );
      })
      .catch(function () {
        var thinking = document.querySelector(".ai-message.bot:last-child");
  ...
  function addUserMessage(text) {
    ...
    if (!container) return;
    container.innerHTML +=
      '<div class="ai-message user">' + esc(text) + "</div>";
  ...
  function addBotMessage(text) {
    ...
    if (!container) return;
    container.innerHTML +=
      '<div class="ai-message bot">' + esc(text) + "</div>";
    container.scrollTop = container.scrollHeight;
  }
