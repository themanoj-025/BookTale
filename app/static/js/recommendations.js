...
(function () {
    ...
    fetch("/api/recommendations/me")
      ...
      .then(function (data) {
        if (!data || !data.books || !data.books.length) {
          container.innerHTML =
            '<div class="empty-state"><div class="empty-icon">📚</div><div class="empty-title">No personalized recommendations yet</div><div class="empty-desc">Read and rate more books to get AI-powered suggestions.</div></div>';
          return;
        }
        ...
        container.innerHTML =
          ...
          data.books
            ...
            .map(function (book) {
              var strategy =
                data.strategy === "personalized" ? "✨ AI Pick" : "🔥 Popular";
              return (
                '<article class="book-card" aria-label="Book: ' +
                esc(book.title) +
                '">' +
                '<figure><a href="/books/' +
                ...
                "</a>" +
                '<div class="ai-badge" style="position:absolute;top:6px;left:6px;" title="Recommended based on your reading history">' +
                strategy +
                "</div>" +
                "</figure>" +
                '<figcaption><h3 style="font-size:.85rem;font-weight:700;margin-top:.3rem;"><a href="/books/' +
                (book.book_id || book.id) +
                '">' +
                esc(book.title) +
                "</a></h3>" +
                '<p style="font-size:.75rem;color:var(--text-muted)">' +
                esc(book.authors || book.author || "Unknown") +
                "</p>" +
                "</figcaption></article>"
              );
            })
      ...
      .catch(function (err) {
        ...
        container.innerHTML =
          '<div class="empty-state"><div class="empty-icon">⚠️</div><div class="empty-title">Could not load recommendations</div></div>';
      });
  }
    ...
    fetch("/api/books/trending")
      ...
      .then(function (books) {
        if (!books || !books.length) {
          container.innerHTML =
            '<div class="text-center text-muted small py-3">No trending books yet</div>';
          return;
        }
        container.innerHTML =
          ...
          books
            ...
            .map(function (book) {
              return (
                '<div class="book-card" style="width:160px;flex-shrink:0;">' +
                '<figure><a href="/books/' +
                (book.book_id || book.id) +
                '">' +
                ...
                "</a></figure>" +
                '<figcaption><div style="font-size:.8rem;font-weight:600;margin-top:.25rem;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' +
                esc(book.title) +
                "</div>" +
      ...
      .catch(function () {
        if (container)
          container.innerHTML =
            '<div class="text-center text-muted small py-3">No data</div>';
      });
  }
  ...
  function showSkeleton(container) {
    container.innerHTML =
      '<div class="book-grid">' +
      '<div class="skeleton" style="height:280px;border-radius:10px;"></div>'.repeat(
        6,
      ) +
      "</div>";
  }
  ...
  function init() {
    if (
      document.getElementById("rec-personalized") ||
