...
from app.routes.feature_shared import _js_str, _storage, _wishlist, h
...
def register_wishlist_routes(app, login_required, admin_required, render_page, _rate_limit) -> None:
    ...
    def wishlist_page() -> str:
        ...
        STAT_BAR = (
            ...
            + str(stats["total"])
            + '</div><div class="desc">Total</div></div>'
            '<div class="stat-item"><div class="num text-warning">'
            + str(stats["pending"])
            + '</div><div class="desc">Pending</div></div>'
            '<div class="stat-item"><div class="num text-success">'
            + str(stats["approved"])
            + '</div><div class="desc">Approved</div></div>'
            '<div class="stat-item"><div class="num text-danger">'
            + str(stats["rejected"])
            + '</div><div class="desc">Rejected</div></div>'
            '<div class="stat-item"><div class="num">'
            + str(stats["unique_suggesters"])
            + '</div><div class="desc">Suggesters</div></div>'
            "</div>"
        )
        ...
        for s in suggestions:
            ...
            if session.get("role") == "admin" and s["status"] == "pending":
                ...
                mod_buttons = (
                    '<button class="btn btn-sm btn-success" onclick="moderateSuggestion(\''
                    + _sid
                    + "','approved')\"><i class=\"bi bi-check-lg\"></i> Approve</button>"
                    '<button class="btn btn-sm btn-danger" onclick="moderateSuggestion(\''
                    + _sid
                    + "','rejected')\"><i class=\"bi bi-x-lg\"></i> Reject</button>"
                )
            cat_badge = (
            ...
            SUGGESTIONS_HTML += (
                ...
                + _js_str(s["suggestion_id"])
                + "','up',this)\"><i class=\"bi bi-arrow-up-short\"></i></button>"
                '<span class="vote-score ' + score_cls + '">' + str(score) + "</span>"
                '<button class="vote-btn '
                + down_class
                + '" onclick="voteSuggestion(\''
                + _js_str(s["suggestion_id"])
                + "','down',this)\"><i class=\"bi bi-arrow-down-short\"></i></button>"
                "</div>"
                '<div class="flex-grow-1" style="min-width:0;">'
                '<div class="d-flex justify-content-between align-items-start">'
                '<div><h6 class="fw-bold mb-0">' + h(s["title"]) + "</h6>"
                '<small class="text-muted">'
                + (h(s.get("author", "")) if s.get("author") else "")
                + "</small></div>"
                + sb
                + "</div>"
                + reason_html
                ...
                + mod_buttons
                + '<button class="btn btn-sm btn-outline" onclick="showSuggestionComments(\''
                + _js_str(s["suggestion_id"])
                + '\')"><i class="bi bi-chat"></i> Comment</button>'
                "</div></div></div></div>"
            )
        ...
        STATUS_TABS = (
            ...
            '<a href="/wishlist?status=pending" class="feed-tab '
            + active_pending
            + '">⏳ Pending</a>'
            '<a href="/wishlist?status=approved" class="feed-tab '
            + active_approved
            + '">✅ Approved</a>'
            '<a href="/wishlist?status=rejected" class="feed-tab '
            + active_rejected
            + '">❌ Rejected</a>'
            "</div>"
        )
        ...
        CONTENT = (
            ...
            "</div>" + STAT_BAR + STATUS_TABS + '<div class="row">'
