...
class TestReadingProgress:
    def test_update_progress(self, mgr: ReadingProgress) -> None:
        with (
            patch.object(mgr, "_load_progress", return_value={}),
            patch.object(mgr, "_save_progress"),
        ):
    ...
    def test_add_bookmark(self, mgr: ReadingProgress) -> None:
        with (
            patch.object(mgr, "_load_bookmarks", return_value=[]),
            patch.object(mgr, "_save_bookmarks"),
        ):
    ...
    def test_delete_bookmark(self, mgr: ReadingProgress) -> None:
        ...
        with (
            patch.object(mgr, "_load_bookmarks", return_value=bms),
            patch.object(mgr, "_save_bookmarks"),
        ):
